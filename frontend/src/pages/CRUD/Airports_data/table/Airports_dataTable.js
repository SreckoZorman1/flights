...
import actions from 'actions/airports_data/airports_dataListActions';
import actionsAirports_data from 'actions/airports_data/airports_dataFormActions';
...
const Airports_dataTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'airport_name', title: 'airport_name' },
    { label: 'city', title: 'city' },
    { label: 'timezone', title: 'timezone' },
    { label: 'airport_code', title: 'airport_code' },
  ]);
  ...
  const count = useSelector((store) => store.airports_data.list.count);
  const modalOpen = useSelector((store) => store.airports_data.list.modalOpen);
  const rows = useSelector((store) => store.airports_data.list.rows);
  const idToDelete = useSelector(
    (store) => store.airports_data.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsAirports_data.doUpdate(id, data));
  };
  ...
  const sendAirports_dataCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('airports_data/bulk-import', data, {
        headers: {
  ...
  const getAirports_dataCSV = async () => {
    const response = await axios({
      url: '/airports_data?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'airports_dataCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Airports_data')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/airports_data/new' className={classes.element}>
...

        {
          <div>
            ...
            >
              API documentation for airports_data
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Airports_dataTable;
