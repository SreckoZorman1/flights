...
// eslint-disable-next-line
import * as airports_dataDataFormat from 'pages/CRUD/Airports_data/table/Airports_dataDataFormatters';

import actions from 'actions/flights/flightsListActions';
import actionsFlights from 'actions/flights/flightsFormActions';
...
const FlightsTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'aircraft_code', title: 'aircraft_code' },
    { label: 'flight_no', title: 'flight_no' },

    { label: 'Departure Airport', title: 'departure_airport' },
    { label: 'Arrival Airport', title: 'arrival_airport' },
  ]);
  ...
  const count = useSelector((store) => store.flights.list.count);
  const modalOpen = useSelector((store) => store.flights.list.modalOpen);
  const rows = useSelector((store) => store.flights.list.rows);
  const idToDelete = useSelector((store) => store.flights.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsFlights.doUpdate(id, data));
  };
  ...
  const sendFlightsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('flights/bulk-import', data, {
        headers: {
  ...
  const columns = [
    {
      ...
      valueGetter: (GridValueGetterParams) =>
        new Date(GridValueGetterParams.row.scheduled_departure),

      headerName: 'scheduled_departure',
    },

    {
      ...
      type: 'dateTime',
      valueGetter: (GridValueGetterParams) =>
        new Date(GridValueGetterParams.row.scheduled_arrival),

      headerName: 'scheduled_arrival',
    },
...

    {
      ...
      type: 'dateTime',
      valueGetter: (GridValueGetterParams) =>
        new Date(GridValueGetterParams.row.actual_departure),

      headerName: 'actual_departure',
    },

    {
      ...
      type: 'dateTime',
      valueGetter: (GridValueGetterParams) =>
        new Date(GridValueGetterParams.row.actual_arrival),

      headerName: 'actual_arrival',
    },
...

    {
      ...
      sortable: false,
      renderCell: (params) =>
        airports_dataDataFormat.listFormatter(
      ...
      valueFormatter: ({ value }) =>
        dataFormat.airports_dataOneListFormatterEdit(value),
  ...
  return (
    <div>
      <Widget title={humanize('Flights')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/flights/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Airports_data'
                  : 'http://localhost:8080/api-docs/#/Airports_data'
              }
            >
              API documentation for flights
            </LinkMaterial>
          </div>
        }
      </Widget>
