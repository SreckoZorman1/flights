...
function airports_dataManyListFormatter(val) {
  if (!val || !val.length) return [];
  return val.map((item) => item.id);
}
function airports_dataOneListFormatter(val) {
  if (!val) return '';
  return val.id;
}
function airports_dataManyListFormatterEdit(val) {
  if (!val || !val.length) return [];
  return val.map((item) => {
    return { id: item.id, label: item.id };
  });
}
function airports_dataOneListFormatterEdit(val) {
  if (!val) return '';
...
export {
  ...
  airports_dataOneListFormatterEdit,
  airports_dataManyListFormatterEdit,
  airports_dataOneListFormatter,
  airports_dataManyListFormatter,
};
