...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/flights/autocomplete`, { params });
  return response.data;
}

const FlightsSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.flights.hasPermissionToCreate,
});

export default connect(select)(FlightsSelectItem);
