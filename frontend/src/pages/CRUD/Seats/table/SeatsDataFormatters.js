...
function aircrafts_dataManyListFormatter(val) {
  if (!val || !val.length) return [];
  return val.map((item) => item.id);
}
function aircrafts_dataOneListFormatter(val) {
  if (!val) return '';
  return val.id;
}
function aircrafts_dataManyListFormatterEdit(val) {
  if (!val || !val.length) return [];
  return val.map((item) => {
    return { id: item.id, label: item.id };
  });
}
function aircrafts_dataOneListFormatterEdit(val) {
  if (!val) return '';
...
export {
  ...
  aircrafts_dataOneListFormatterEdit,
  aircrafts_dataManyListFormatterEdit,
  aircrafts_dataOneListFormatter,
  aircrafts_dataManyListFormatter,
};
