...
// eslint-disable-next-line
import * as aircrafts_dataDataFormat from 'pages/CRUD/Aircrafts_data/table/Aircrafts_dataDataFormatters';

import actions from 'actions/seats/seatsListActions';
import actionsSeats from 'actions/seats/seatsFormActions';
...
const SeatsTable = () => {
  ...
  const [filters, setFilters] = useState([
    { label: 'seat_no', title: 'seat_no' },
    { label: 'fare_conditions', title: 'fare_conditions' },

    { label: 'Aircraft code', title: 'aircraft_code' },
  ]);
  ...
  const count = useSelector((store) => store.seats.list.count);
  const modalOpen = useSelector((store) => store.seats.list.modalOpen);
  const rows = useSelector((store) => store.seats.list.rows);
  const idToDelete = useSelector((store) => store.seats.list.idToDelete);
  ...
  const update = (id, data) => {
    dispatch(actionsSeats.doUpdate(id, data));
  };
  ...
  const sendSeatsCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('seats/bulk-import', data, {
        headers: {
  ...
  const getSeatsCSV = async () => {
    const response = await axios({
      url: '/seats?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
  ...
  const columns = [
...

    {
      ...
      sortable: false,
      renderCell: (params) =>
        aircrafts_dataDataFormat.listFormatter(
      ...
      valueFormatter: ({ value }) =>
        dataFormat.aircrafts_dataOneListFormatterEdit(value),
      renderEditCell: (params) => {
        return (
          <DataGridSinglSelect
            {...params}
            entityName={'aircrafts_data'}
            nameRow={'id'}
          />
        );
      },
  ...
  return (
    <div>
      <Widget title={humanize('Seats')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/seats/new' className={classes.element}>
