...
import actions from 'actions/aircrafts_data/aircrafts_dataListActions';
import actionsAircrafts_data from 'actions/aircrafts_data/aircrafts_dataFormActions';
...
const Aircrafts_dataTable = () => {
  ...
  const [filters, setFilters] = useState([
    ...
    { label: 'constraint', title: 'constraint' },
    { label: 'aircraft_code', title: 'aircraft_code' },
    { label: 'range', title: 'range', number: 'true' },
  ]);
  ...
  const count = useSelector((store) => store.aircrafts_data.list.count);
  const modalOpen = useSelector((store) => store.aircrafts_data.list.modalOpen);
  const rows = useSelector((store) => store.aircrafts_data.list.rows);
  const idToDelete = useSelector(
    (store) => store.aircrafts_data.list.idToDelete,
  );
  ...
  const update = (id, data) => {
    dispatch(actionsAircrafts_data.doUpdate(id, data));
  };
  ...
  const sendAircrafts_dataCSV = async () => {
    const data = new FormData();
    data.append('file', file[0]);
    data.append('filename', file[0].name);

    try {
      const result = await axios.post('aircrafts_data/bulk-import', data, {
        headers: {
  ...
  const getAircrafts_dataCSV = async () => {
    const response = await axios({
      url: '/aircrafts_data?filetype=csv',
      method: 'GET',
      responseType: 'blob',
    });
    ...
    link.download = 'aircrafts_dataCSV.csv';
    link.click();
  };
  ...
  return (
    <div>
      <Widget title={humanize('Aircrafts_data')} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to='/admin/aircrafts_data/new' className={classes.element}>
            ...
            <LinkMaterial
              ...
              href={
                process.env.NODE_ENV === 'production'
                  ? window.location.origin + '/api-docs/#/Aircrafts_data'
                  : 'http://localhost:8080/api-docs/#/Aircrafts_data'
              }
            >
              API documentation for aircrafts_data
            </LinkMaterial>
          </div>
        }
      </Widget>
...
export default Aircrafts_dataTable;
