...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/aircrafts_data/autocomplete`, { params });
  return response.data;
}

const Aircrafts_dataSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.aircrafts_data.hasPermissionToCreate,
});

export default connect(select)(Aircrafts_dataSelectItem);
